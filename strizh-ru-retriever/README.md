...
Один харнесс, MIRACL-ru dev, recall@10 на **отфильтрованной подвыборке** (758 из 1000
запросов, у которых ни один gold-пассаж не совпадал по первым 200 символам с train-
позитивами donor-линии — фильтр известных прямых совпадений, не полный near-duplicate
audit). Донор видел часть dev-пассажей, поэтому полный `0.80` потенциально оптимистичен;
подвыборка при этом просто труднее, так что разницу нельзя читать как «цену утечки».
...
3. **Hard-negatives от BGE-M3.** Учитель мультиязычный, негативы майнятся из смешанного
   RU/EN пула (майнер не гарантирует, что негатив кросс-язычный); финальный контрастивный
   проход (`mine_bge.py`, `train_v2i2.py`).
