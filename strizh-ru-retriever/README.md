...
маленькая модель быстрее завершает первую стадию поиска (прямое влияние на TTFT/tok/s
одновременно работающей LLM не измерялось). Drop-in без префиксов, работает как первая
стадия в гибридном (вектор+BM25) поиске под reranker.
...
на отфильтрованной подвыборке (см. «Метрики»), а не на полном наборе.
...
| `mine_bge.py` | hard-negatives от BGE-M3 (смешанный RU/EN пул) |
