...
| [`results-flags-control-strix-2026-07-28.txt`](results-flags-control-strix-2026-07-28.txt) | контроль асимметрии флагов серверов: bge-m3 с флагами STRIZH (`-b/-ub 8192`, `-c 65536`) против своих штатных | — |
| `prompt_identity_check.py` + [`results-prompt-identity-strix-2026-07-28.txt`](results-prompt-identity-strix-2026-07-28.txt) | SHA-256 итогового промпта по каждому запросу: доказывает, что на одном запросе обе модели собрали побайтово идентичный промпт (равной длины недостаточно) | — |
| `analyze_prompt_tokens.py` | разбор лога токенов: сырая и свёрнутая регрессии, помодельные прямые, знаковый тест, естественный контроль | — |
