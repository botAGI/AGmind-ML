...
3. Сильный учитель (**DeepSeek-V4-Flash**, OpenAI-совместимый эндпоинт, grammar-constrained `guided_json`) размечает индексы границ + topic. Примечание: в ранней версии доки значился `temperature=0`; фактически v1-пайплайн вызывал разметку с `temp=0.2` (дефолт `label()` в `generate.py`) — known discrepancy, компенсируется гейтами, дедупом и ретраями.
4. **Гейты качества** (каждый пример обязан пройти): валидный JSON; 1–60 границ; кириллица > 0.6; ни одного не-последнего чанка < 8 слов; topic — русский ≤ 200 симв.; точный MD5-дедуп по первым 500 символам.
