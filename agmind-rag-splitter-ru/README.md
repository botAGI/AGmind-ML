...
Исходный snapshot **N=300** из holdout-1500 (согласие границ с метками учителя), greedy-декодинг:
...
Перепроверка на полном **holdout-1500** (2026-06-26, raw stdout — в [`eval/`](eval/)):
...
- На полном holdout GGUF/Vulkan практически равен HF-модели → **токенайзер и Q5 сохраняют качество**.
...
- **Кириллическая база.** `T-lite-it-2.1` (continued-pretrain Qwen3-8B от T-Bank) несёт переработанный токенайзер. Замер на русском: **1.74 ток/слово** против **3.17** у Llama-2 (токенайзер датского оригинала) и 2.71 у ванильного Qwen2.5 — T-lite почти вдвое компактнее на кириллице (см. [eval/results.md](eval/results.md)).
...
- **Дистилляция от учителя.** Сильная модель размечает, где резать; без ручной разметки. Метки фильтруют жёсткие гейты, точный MD5-дедуп и ретраи (multi-sample voting / self-consistency в v1 не запускалось).
